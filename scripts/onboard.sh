#!/usr/bin/env bash
#
# ERP Orchestrator Project Onboarding
# Interactive setup for using erp-orch with a new project
#
# Usage:
#   erp-orch-onboard
#   # or
...
# Default paths
CONFIG_DIR="${HOME}/.config/erp-orchestrator"
CONFIG_FILE="${CONFIG_DIR}/config.toml"
DATA_DIR="${HOME}/.erp-orchestrator"
...
check_prerequisites() {
    ...
    # erp-orch
    if command -v erp-orch &> /dev/null; then
        success "erp-orch: $(erp-orch --version 2>/dev/null || echo 'found')"
    else
        warn "erp-orch not in PATH (using local if available)"
    fi
...
create_config() {
    ...
    cat > "$CONFIG_FILE" << EOF
# ERP Orchestrator Configuration
...
initial_sync() {
    ...
    if confirm "Run initial task sync?" "y"; then
        if command -v erp-orch &> /dev/null; then
            ...
            warn "erp-orch not in PATH, skipping sync"
            echo "Run 'erp-orch sync' manually after adding to PATH"
        fi
    fi
}
...
print_summary() {
    ...
    echo "     erp-orch start              # Start ready tasks"
    echo "     erp-orch tui                # TUI dashboard"
    echo "     erp-orch serve              # Web UI at http://${WEB_HOST}:${WEB_PORT}"
    echo ""
    echo "  4. Monitor progress:"
    echo "     erp-orch status             # Quick summary"
    echo "     erp-orch list               # All tasks"
    echo ""
...
main() {
    echo ""
    echo "  ╔═══════════════════════════════════════╗"
    echo "  ║   ERP Orchestrator Project Onboarding  ║"
    echo "  ╚═══════════════════════════════════════╝"
    echo ""
